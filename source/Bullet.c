...
void BulletInit(Bullet *self)
{
    ...
    self->type = BulletType_UndefinedBullet;
}

void BulletInitBulletArray(Bullet bullet_array[], const int bullet_array_len)
{
...
int BulletGetNumberAliveBulletsInBulletArray(Bullet bullet_array[], const int bullet_array_len)
{
    int num = 0;
...
void BulletHandleBulletArray(Bullet bullet_array[], const int bullet_array_len, int boundary_rectangle[4])
{
    ...
    Entity *player)
{
    ...
    int player_collision_type = BulletType_PlayerBullet;
    ...
    for (int bullet_index = 0; bullet_index < bullet_array_len; bullet_index++)
    {
        ...
        if (bullet_array[bullet_index].alive)
        {
            ...
            if (bullet_array[bullet_index].type == BulletType_PlayerBullet)
            {
                // For each enemy
            ...
            else if ((int) bullet_array[bullet_index].type > 0)
            {
    ...
    for (int i = 0; i < enemy_array_len; i++)
    {
        ...
        if (enemy_array[i].dead && enemy_array[i].counter == 11)
        {
            ...
            const float angle = GetAngleFromOriginTo(
                (float) (player_center[0] - enemy_center[0]),
                (float) (player_center[1] - enemy_center[1])
            );

            // Bullet time
            BulletSetupInBulletArray(
                bullet_array, bullet_array_len,
                (float) (enemy_center[0] - 1),
                (float) (enemy_center[1] - 1),
                3, 3,
                angle,
                1,
                320,
                1,
                BulletType_DeathBullet
            );
            BulletSetupInBulletArray(
                bullet_array, bullet_array_len,
                (float) (enemy_center[0] - 1),
...
void BulletDraw(const Bullet *self, const int id_offset, u16 *bullet_gfx_mem[][FRAMES_PER_BULLET])
{
    oamSet(
        &oamMain,
