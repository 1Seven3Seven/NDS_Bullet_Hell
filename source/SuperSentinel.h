...
typedef struct _SSLaserStruct_s
{
    int Initialised;
    int StartIndex;
...
typedef struct _SSSuperSentinelInformation_s
{
    /// The number of bullets fired in the current salvo.
    int BulletsFired;
    ...
    ///      2  = Move the lasers back to the main body.
    int MoveLasers;

    ///
    SSLaserStruct Lasers[2];
} _SSSuperSentinelInformation;

/// Contains information about the super sentinel, not necessary to touch directly.
/// Initialised when SSSetup is called.
extern _SSSuperSentinelInformation SSSuperSentinelInformation;
...
///
/// Assumes the enemy array is at least a size of 3.
void SSSetup(Entity enemy_array[], int enemy_array_len);

/// Sets the position for the top left of the main body and the rest of the parts accordingly.
void SSSetPosition(Entity enemy_array[], float x, float y);

/// Gets the health of the boss.
/// Health is split between the 3 parts, so use this to for easier retrieval.
...
///
///      1. The boss is dead and there are no enemy bullets.
///          Returns 1.
///
///      2. The player dies.
///          Returns 0.
